...
import { getContract, TokenExchange } from "config/constants/contracts";
import {
  Metadata,
  PROGRAM_ADDRESS as MPL_PROGRAM_ID,
} from "@metaplex-foundation/mpl-token-metadata";

const escrowContract = getContract("solana");

const IDL = escrowContract.IDL;
...
type _AllowTokenStateType =
  IdlAccounts<TokenExchange>[typeof _AllowTokenStateTypeName];
...
type EOfferCompletedType =
  IdlEvents<TokenExchange>[typeof _EOfferCompletedName];
...
type ETokenDisAllowedType =
  IdlEvents<TokenExchange>[typeof _ETokenDisAllowedName];

const log = console.log;
const systemProgram = web3.SystemProgram.programId;
const Seeds = {
  SEED_MAIN_STATE: utf8.encode("main_state"),
  SEED_ALLOWED_TOKEN: utf8.encode("allowed_token"),
  SEED_OFFER: utf8.encode("offer"),
};

const VITE_SOLANA_MAINNET_RPC = import.meta.env.VITE_SOLANA_MAINNET_RPC;
const mainnetRPC = VITE_SOLANA_MAINNET_RPC
...
export class Connectivity {
  ...
  constructor(_wallet: WalletContextState, chainId: any) {
    const RPC = Number(chainId) === 101 ? mainnetRPC : devnetRPC;
    ...
    this.programId = new web3.PublicKey(escrowContract.address);
    ...
    // this.feeReceiver = new web3.PublicKey(escrowContract.feeReceiver);
    this.mainStateAccount = web3.PublicKey.findProgramAddressSync(
  ...
  async getMainStateInfo() {
    try {
      const info = await this.program.account.mainState.fetch(
      ...
      log({ error });
      throw "Failed to get mainState info";
    }
  }

  async getFeeReceiver() {
    const { feeReceiver } = await this.getMainStateInfo();

    return feeReceiver;
  };


  async tranferOwnerShip(newOwner: string | web3.PublicKey) {
    this.txis = [];
    const owner = this.wallet.publicKey;
    if (owner == null) throw "Wallet not found !";
    if (typeof newOwner == "string") newOwner = new web3.PublicKey(newOwner);
  ...
  async getTokenInfo(token: web3.PublicKey | string) {
    ...
    try {
      ...
      const metadataAccount = web3.PublicKey.findProgramAddressSync(
        [utf8.encode("metadata"), MPL_ID.toBuffer(), token.toBuffer()],
        MPL_ID
      )[0];
      ...
      try {
        ...
        return {
          name: tokenMetadata.data.name.split("\x00")[0],
          symbol: tokenMetadata.data.symbol.split("\x00")[0],
          decimals,
        };
      } catch {
        //NAME NOT SET
  ...
  _parseMainState(
    ...
    id: web3.PublicKey = null
  ) {
    const state = this.program.coder.accounts.decode<_MainStateType>(
      _MainStateTypeName,
      data
    );
    if (verify) {
  ...
  _parseAllowedTokenState(
    data: Buffer,
    verify: boolean = false,
    id: web3.PublicKey = null
  ) {
    const res = this.program.coder.accounts.decode<_AllowTokenStateType>(
      _AllowTokenStateTypeName,
      data
    );
    if (verify) {
      const _allowedTokenStateAccountStr = this.__getAllowTokenCheckAccount(
        res.mint
      ).toBase58();
  ...
  async _parseOfferState(
    data: Buffer,
    verify: boolean = false,
    id: web3.PublicKey = null
  ) {
    const state = this.program.coder.accounts.decode<_OfferStateType>(
      _OfferStateTypeName,
      data
    );
    ...
    const requestedTokenDecimal = await this._getTokenDecimal(
      state.requestedToken
    );
    ...
    return {
      ...
      offeredTokenAmount:
        state.offeredAmount.toNumber() / 10 ** offeredTokenDecimal,
      minRequestedAmount:
        state.minRequestedAmount.toNumber() / 10 ** requestedTokenDecimal,
      requestedToken: state.requestedToken.toBase58(),
      requestedTokenAmount:
        state.requestedAmount.toNumber() / 10 ** requestedTokenDecimal,
    };
  }
  ...
  async initOfferState(
    ...
    const offerStateAccount = this.__getOfferStateAccount(
      offeror,
      offeredToken,
      requestedToken,
      initTime
    );
    const offerStateAccountAta = getAssociatedTokenAddressSync(
      offeredToken,
      offerStateAccount,
      true
    );

    const offeredTokenAllowedChecker =
      this.__getAllowTokenCheckAccount(offeredToken);
    const requestedTokenAllowedChecker =
      this.__getAllowTokenCheckAccount(requestedToken);

    log("initalizeing acount");
    const ix = await this.program.methods
      .initOfferState(new BN(initTime))
      .accounts({
        offeror,
        offerStateAccount,
  ...
  async _getAllProgramAccounts() {
    ...
    for (let i of res) {
      ...
      try {
        if (
          this.program.coder.accounts.size(IDL.accounts[2]) + 7 ===
          accountInfo.data.length
        ) {
          const res = await this._parseOfferState(
            accountInfo.data,
            true,
            i.pubkey
          );
          ...
          const tokenId = this._parseAllowedTokenState(
            accountInfo.data,
            true,
            i.pubkey
          );
  ...
  async createOffer(
    ...
    offeredTokenAmount = Connectivity.calculateNonDecimalValue(
      offeredTokenAmount,
      offeredTokenDecimal
    );
    requestedTokenAmount = Connectivity.calculateNonDecimalValue(
      requestedTokenAmount,
      requestedTokenDecimal
    );
    minRequestedAmount = Connectivity.calculateNonDecimalValue(
      minRequestedAmount,
      requestedTokenDecimal
    );
    ...
    const ix = await this.program.methods
      .createOffer(
        new BN(offeredTokenAmount),
        new BN(requestedTokenAmount),
        new BN(minRequestedAmount)
      )
      .accounts({
        offeror,
        offerorAta,
  ...
  async acceptOffer(offerId: web3.PublicKey, amount: number) {
    const acceptor = this.wallet.publicKey;
    if (acceptor == null) throw new Error("Wallet not found !");
    ...
    const acceptorOfferedTokenAta = await this._getOrCreateTokenAccount(
      offeredToken,
      acceptor
    );
    const acceptorRequestedTokenAta = await this._getOrCreateTokenAccount(
      requestedToken,
      acceptor
    );
    const offerorRequestedTokenAta = await this._getOrCreateTokenAccount(
      requestedToken,
      offeror
    );

    const _offerId = new web3.PublicKey(offerId);
    ...
    const requestedTokenDecimal = await this._getTokenDecimal(
      offerState.requestedToken
    );
    amount = Connectivity.calculateNonDecimalValue(
      amount,
      requestedTokenDecimal
    );

    const ix = await this.program.methods
      .acceptOffer(new BN(amount))
      .accounts({
        acceptor,
  ...
  async editOffer(input: EditOfferInput) {
    const offeror = this.wallet.publicKey;
    if (offeror == null) throw new Error("Wallet not found");
    const _offerId = new web3.PublicKey(input.offerId);
    const offerStateAccountInfo = await this.connection.getAccountInfo(
      _offerId
    );
    if (offerStateAccountInfo == null) throw new Error("offer not found !");
    ...
    const requestedTokenDecimal = await this._getTokenDecimal(requestedToken);
    const newRequestedTokenAmount = Connectivity.calculateNonDecimalValue(
      input.newRequestedTokenAmount,
      requestedTokenDecimal
    );
    const newMinRequestedTokenAmount = Connectivity.calculateNonDecimalValue(
      input.newMinRequestedTokenAmount,
      requestedTokenDecimal
    );
  ...
  async closeOffer(offerId: web3.PublicKey) {
    const offeror = this.wallet.publicKey;
    if (offeror == null) throw new Error("Wallet not found");
  ...
  async _allowToken(tokenId: web3.PublicKey | string) {
    const owner = this.wallet.publicKey;
    if (owner == null) throw new Error("Wallet not found !");
  ...
  async _listenEvents() {
    this.program.addEventListener(
      _EOfferCreatedName,
      (event: EOfferCreatedType, slot, signature) => {
        const parseValue = {
        ...
        log("event : ", { OfferCreated: parseValue, slot, signature });
      }
    );

    this.program.addEventListener(
      _EOfferAcceptedName,
      (event: EOfferAcceptedType, slot, signature) => {
        const parseValue = {
          offerId: event.offerId.toBase58(),
    ...
    this.program.addEventListener(
      _EOfferCompletedName,
      (event: EOfferCompletedType, slot, signature) => {
    ...
    this.program.addEventListener(
      _EOfferRevokedName,
      (event: EOfferRevokedType, slot, signature) => {
    ...
    this.program.addEventListener(
      _ETokenAllowedName,
      (event: ETokenAllowedType, slot, signature) => {
        log("event: ", { TokenAllowed: event.tokenId.toBase58() });
      }
    );

    this.program.addEventListener(
      _ETokenDisAllowedName,
      (event: ETokenDisAllowedType, slot, signature) => {
